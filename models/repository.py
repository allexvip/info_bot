...
class RepositorySQLite(object):
    ...
    def get_all_votes_by_chat(self, chat_id):
        ...
        for v in self.cursor.execute(sql).fetchall():
            result.append(Vote(v[0], v[1], v[2], v[3], v[4], v[5], self.get_deputy_by_id(v[3])))
        return result

    def get_unconfirmed_votes_by_chat(self, chat_id):
        result = []
        sql = 'select * from votes where chat_id={0} and confirmed=0'.format(chat_id)
        for v in self.cursor.execute(sql).fetchall():
            result.append(Vote(v[0], v[1], v[2], v[3], v[4], v[5], self.get_deputy_by_id(v[3])))
        return result

    def get_all_deputys(self):
        pass
